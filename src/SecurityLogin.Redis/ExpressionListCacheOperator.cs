...
namespace SecurityLogin.Redis
{
    public class ExpressionListCacheOperator : ComplexCacheOperator,IListCacheOperator
    {
        ...
        private ICacheColumn[] columns;
        private ICacheColumn[] noNextsColumns;

        protected override void OnBuild()
        {
            var cls = new List<ICacheColumn>();
            IEnumerable<ICacheColumn> current = RedisColumns;
            while (current.Any())
            {
            ...
            columns = cls.ToArray();
            noNextsColumns = columns.Where(x => x.Nexts == null || x.Nexts.Count == 0).ToArray();
        ...
        private IEnumerable<Expression> AotWriteAll(Expression instance, IEnumerable<ICacheColumn> columns, Expression map)
        {
            ...
            {
                var index = Array.IndexOf(noNextsColumns, column);
                if (index != -1)
                {
